...
<body class="bg-blue-gray-50" x-data="initApp()" x-init="initDatabase(), loadData()">
  <!-- noprint-area -->
  <div class="hide-print flex flex-row h-screen antialiased text-blue-gray-800">
    ...
    <!-- <div class="flex flex-row w-auto flex-shrink-0 pl-4 pr-2 py-4">
      <div class="flex flex-col items-center py-4 flex-shrink-0 w-20 bg-blue-800 rounded-3xl">
        <a href="#"
           class="flex items-center justify-center h-12 w-12 bg-blue-50 text-blue-700 rounded-full">
           <img src="img/logo/logo_arah_melangkah.png" alt="Arah Melangkah" width="25px" height="25px" viewBox="0 0 24 24">
        ...
        <ul class="flex flex-col space-y-2 mt-12">
          <li>
            <a href="#"
              ...
              <span
                ...
                x-bind:class="{
                  'hover:bg-blue-700 text-blue-100': activeMenu !== 'pos',
                  'bg-blue-600 shadow-lg text-white': activeMenu === 'pos',
                }"
              >
          ...
          <li>
            <a href="#"
               class="flex items-center">
              <span class="flex items-center justify-center text-blue-100 hover:bg-blue-700 h-12 w-12 rounded-2xl">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- page content -->
    <div class="flex-grow flex pl-1">
      <!-- store menu -->
      <div class="flex flex-col bg-blue-gray-50 h-full w-full py-4">
        <div class="grid grid-rows-1 grid-flow-col px-2 py-2 relative">
          <div class="font-semibold text-lg pl-2">
            <h2>Order No. #001</h2>
          </div>
          <div class="text-lg pr-2 text-right">
            <small>Today, 02 May 2021, 01:34 PM</small>
          </div>
        </div>
        <div class="flex px-2 flex-row relative">
          <div class="absolute left-5 top-3 px-2 py-2 rounded-full bg-blue-800 text-white">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </div>
          <input type="text" class="bg-white rounded-3xl shadow text-md full w-full h-16 py-4 pl-16 transition-shadow focus:shadow-2xl focus:outline-none" placeholder="Search product ..." x-model="keyword" />
        </div>
          ...
          <div class="h-full overflow-y-auto px-2">
            ...
            <div x-show="filteredProducts().length" class="grid grid-cols-4 gap-4 pb-3">
              <template x-for="product in filteredProducts()" :key="product.id">
                <div role="button" class="select-none cursor-pointer transition-shadow overflow-hidden rounded-2xl bg-white shadow hover:shadow-lg" :title="product.product_name" x-on:click="addToCart(product)">
                  <img :src="'uploads/product/' + product.product_photo" :alt="product.product_name">
                  <div class="flex pb-3 px-3 text-sm -mt-3">
                    <p class="flex-grow truncate mr-1 text-center lg:text-left" x-text="product.product_name"></p>
                    <p class="hidden lg:block truncate font-semibold" x-text="priceFormat(product.product_price)"></p>
                  </div>
                </div>
              </template>
            </div>
      ...
      <!-- right sidebar -->
      <div class="w-8/12 flex flex-col bg-blue-gray-50 h-full bg-white pr-4 pl-2 py-4">
        <div class="bg-white rounded-3xl flex flex-col h-full shadow">
          ...
          <div class="h-16 text-center flex justify-center">
            <div class="pl-8 text-left text-lg py-4 relative">
              ...
              <div x-show="getItemsCount() > 0" class="text-center absolute bg-blue-800 text-white w-5 h-5 text-xs p-0 leading-5 rounded-full -right-2 top-3" x-text="getItemsCount()"></div>
            </div>
            <div class="flex-grow mx-auto text-center text-lg py-4 relative truncate">
              <h5>Customer</h5>
            </div>
            <div class="pr-8 text-right text-lg py-4 relative">
              <!-- trash button -->
          ...
          <div class="select-none h-auto w-full text-center pt-3 pb-4 px-4">
            ...
            <div class="mb-3">
              <div class="flex">
                <p class="text-left ml-2 truncate"><small>Independence Day Disc. 30%</small></p>
                <p class="text-right w-full"><small>(Rp. 7.200)</small></p>
              </div>
              <div class="flex">
                <p class="text-left ml-2 truncate"><small>Cinnamon Rolls Disc.</small></p>
                <p class="text-right w-full"><small>(Rp. 2.500)</small></p>
              </div>
            </div>
            <div class="flex mb-3 text-sm font-semibold text-blue-gray-700">
              <div>TOTAL</div>
            ...
            <!-- <div class="mb-3 text-blue-gray-700 px-3 pt-2 pb-3 rounded-lg bg-blue-gray-50">
              ...
              <div class="grid grid-cols-3 gap-2 mt-2">
                <template x-for="money in moneys">
                  <button x-on:click="addCash(money)" class="bg-white rounded-lg shadow hover:shadow-lg focus:outline-none inline-block px-2 py-1 text-sm truncate">+<span x-text="numberFormat(money)"></span></button>
                </template>
              </div>
            </div>
            <div
            ...
            >
              ...
              <div
                class="text-right flex-grow text-blue-900"
            ...
            <button class="text-white rounded-2xl text-lg w-full py-3 focus:outline-none" x-bind:class="{
                'bg-blue-800 hover:bg-blue-900': submitable(),
      ...
      <div x-show="isShowModalReceipt" class="w-96 rounded-3xl bg-white shadow-xl overflow-hidden z-10" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 transform scale-90" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-90">
        <div id="receipt-content" class="text-left w-full text-sm p-6 overflow-auto">
          <div class="text-center">
            ...
            <h2 class="text-xl font-semibold">ARAH POS</h2>
          ...
          <div>
            ...
            <div class="flex text-xs font-semibold">
              ...
              <input type="text" x-bind:value="numberFormat(cash)" x-on:keyup="updateCash($event.target.value)" placeholder="Insert cash amount" class="w-25 text-right rounded-sm focus:outline-none focus:ring focus:border-blue-300 hide-print ..." />
              <div x-text="priceFormat(cash)" id="cash-printable" class="hidden"></div>
            </div>
            <hr class="my-2">
            <div class="flex text-xs font-semibold">
        ...
        <div class="p-4 w-full">
          <button class="bg-blue-800 text-white text-lg px-4 py-3 rounded-2xl w-full focus:outline-none" x-bind:class="{
              'bg-blue-800 hover:bg-blue-900': proceedable(),
              'bg-blue-gray-200': !proceedable()
            }" :disabled="!proceedable()" x-text="proceedable() ? 'PROCEED': 'INSERT CASH AMOUNT'" x-on:click="printAndProceed()">PROCEED</button>
        </div>
      </div>
    </div>
  </div>
