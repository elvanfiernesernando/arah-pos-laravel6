...
function initApp() {
  const app = {
    ...
    async loadData() {
      const response = await fetch("transaction/getproducts");
    ...
    addToCart(product) {
      ...
      if (index === -1) {
        this.cart.push({
          ...
          image: 'uploads/product/' + product.product_photo,
    ...
    printAndProceed() {
      const cashPrintable = document.getElementById('cash-printable');
      cashPrintable.classList.add("print-area");
      ...
      cashPrintable.classList.add("hidden");
